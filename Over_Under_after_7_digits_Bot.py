...
class DigitBot:
    def __init__(self, config):
        ...
        #self.martingale_stake_list = [0.4, 0.73, 1.27, 2.19, 3.79, 
                                    #  6.56, 11.35, 19.63, 33.96, 58.75]
        self.martingale_stake_list = [0.35, 0.48, 0.82, 1.42, 2.46, 
                                      4.26, 7.36, 12.74, 22.04, 38.13, 
                                      65.96]
    ...
    async def process_digit(self, digit):
        ...
        if self.first_time_2 == True:
            ...
            if self.waiting_for_target and self.appvoed_over\
                ...
                (not self.martingale_started):
                if self.even_trades % 2 == 0:
            ...
            elif self.martingale_started:
                print(f"✓ Martingale trade {self.martingale__trade_number} Placing...")
                self.waiting_for_target = False
                self.trade_active = True
                self.trade_open = True
                if self.trade_type_contract == "under":
                    self.trade_type_contract = "over"
                else:
                    self.trade_type_contract = "under"
    ...
    async def check_contract_result(self):
        ...
        if outcome == "win":
            print(f"✓ Trade WON")
            self.martingale__trade_number = 0
            self.current_stake = self.martingale_stake_list[self.martingale__trade_number]
            ...
            self.martingale__trade_number += 1
            self.current_stake = round(self.martingale_stake_list[self.martingale__trade_number], 2)
            self.martingale_started = True
    ...
    async def ask_to_continue(self):
        ...
        while True:
            ...
            if user_input == 'y':
                ...
                self.total_profit = 0
                self.current_stake = 0.4  # Reset stake
...
async def get_user_config():
    ...
    while True:
        try:
            alter_pattern_length = int(input(f"Alter Pattern length count (default: 6): ") or (6))
            break
        except ValueError:
            print("Invalid input. Please enter a valid integer number.")
    ...
    while True:
        try:
            max_stake = float(input("Enter MAX stake amount (default: 60.0): ") or 60.0)
            if max_stake <= 0:
